...
public class AirlineServiceImpl implements AirlineService {
	...
	public Mono<AirlineResponse> createAirline(AirlineRequest request) {
		...
		return airlineRepository.existsByAirlineCode(code).flatMap(exists -> {
			if (exists) {
				// airline already present -> throw our custom exception
				return Mono.error(new AirlineAlreadyExistsException("Airline with code " + code + " already exists"));
			}
			...
			return airlineRepository.save(airline).map(saved -> {
	...
	public Flux<AirlineResponse> getAllAirlines() {
		return airlineRepository.findAll().map(this::toResponse);
	}
